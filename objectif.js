function main() {
    ...
    firebase.auth().onAuthStateChanged(user => {
        if (user) {
            currentUser = user;
            console.log("User signed in, loading data for Objectifs:", user.uid);
            list.innerHTML = ""; // Clear existing list before loading
            loadItems(user.uid);
            loadDueDate(user.uid);
        } else {
            currentUser = null;
            console.log("User signed out, clearing Objectifs data.");
    ...
    /* ---------- DUE DATE LOGIC ---------- */
    function loadDueDate(uid) {
        db.collection("users").doc(uid).collection("objectifs").doc("dueDate").onSnapshot((doc) => {
            let dueDate;
            if (doc.exists) {
    ...
    function saveDueDate(uid, date) {
        db.collection("users").doc(uid).collection("objectifs").doc("dueDate").set({ date: date.toISOString() })
    ...
    function saveItems(uid) {
        ...
        db.collection("users").doc(uid).collection("objectifs").doc("items").set({ items: items })
            .then(() => console.log("Objectifs items saved to Firestore!"))
            .catch((error) => console.error("Error writing items: ", error));
    }

    function loadItems(uid) {
        db.collection("users").doc(uid).collection("objectifs").doc("items").onSnapshot((doc) => {
            list.innerHTML = ""; // Clear existing list before re-populating
            if (doc.exists) {
                const data = doc.data();
                data.items.forEach(item =>
                    addItem(item.text, item.checked)
                );
                console.log("Objectifs items loaded from Firestore (real-time update).");
            } else {
                console.log("No objectifs items data found for this user.");
            }
        }, (error) => {
            console.error("Error getting document:", error);
        });
    ...
    addBtn.addEventListener("click", () => {
        if (!currentUser) {
            alert("Please sign in to save your objectifs.");
            return;
        }
    ...
    resetBtn.addEventListener("click", () => {
        if (!currentUser) {
            alert("Please sign in to reset your objectifs.");
            return;
        }
