...
function main() {
    ...
    const ITEMS_COLLECTION_NAME = "objectifs";
    ...
    async function saveItemToFirestore(uid, text, checked = false) {
        // Use a unique ID for each item, Firestore can generate one or we can use Date.now()
    ...
    addBtn.addEventListener("click", async () => {
        if (!currentUser) {
            alert("Please sign in to add objectifs.");
            return;
        }
    ...
    resetBtn.addEventListener("click", async () => {
        if (!currentUser) {
            alert("Please sign in to reset objectifs.");
            return;
        }
