function main() {
    ...
    async function loadDueDate(uid) {
        const doc = await db.collection("users").doc(uid).collection("objectifs").doc("dueDate").get();
        let dueDate;
        if (doc.exists) {
    ...
    async function loadItems(uid) {
        const doc = await db.collection("users").doc(uid).collection("objectifs").doc("items").get();
        if (doc.exists) {
            const data = doc.data();
            data.items.forEach(item =>
                addItem(item.text, item.checked)
            );
            console.log("Objectifs items loaded from Firestore.");
        } else {
