...
function main() {
    ...
    function createListItem(itemData, itemId) {
        ...
        orderInput.addEventListener("change", async () => {
            ...
            await saveUserData(currentUser.uid, "quickTaps", itemId, { order: newOrder });
    ...
    async function addItemToFirestore(uid, title, description, order) {
        ...
        await saveUserData(uid, "quickTaps", Date.now().toString(), { title, description, order });
    }

    async function updateItemInFirestore(uid, itemId, title, description) {
        await saveUserData(uid, "quickTaps", itemId, { title, description });
    }

    async function deleteItemFromFirestore(uid, itemId) {
        await deleteUserData(uid, "quickTaps", itemId);
    }

    async function loadList(uid) {
        list.innerHTML = ''; // Clear current list
        const items = await getUserCollection(uid, "quickTaps");
