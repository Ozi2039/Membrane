function main() {
    const button = document.getElementById("quickTaps_button");
    const list = document.getElementById("quickTaps_list");
    const input = document.getElementById("quickTaps_input");
    const descriptionField = document.getElementById("quickTaps_description");
    ...
    firebase.auth().onAuthStateChanged(user => {
        if (user) {
            currentUser = user;
            console.log("User signed in, loading data for:", user.uid);
    ...
    button.addEventListener("click", function () {
        if (!currentUser) {
            alert("Please sign in to save your quick taps.");
            return;
        }
    ...
    function saveList(uid) {
        ...
        db.collection("users").doc(uid).collection("quickTaps").doc("data").set({ items: items })
            .then(() => console.log("Quick Taps saved to Firestore!"))
            .catch((error) => console.error("Error writing document: ", error));
    }

    function loadList(uid) {
        db.collection("users").doc(uid).collection("quickTaps").doc("data").onSnapshot((doc) => {
            ...
            if (doc.exists) {
                ...
                sortList();
                console.log("Quick Taps loaded from Firestore (real-time update).");
            } else {
                console.log("No quick taps data found for this user.");
            }
        }, (error) => {
