...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const header = document.querySelector('header'); // Assuming header is the parent of nav and toggle
    const mainNav = document.getElementById('main-nav');

    if (menuToggle && header && mainNav) {
        menuToggle.addEventListener('click', () => {
            header.classList.toggle('nav-open');
            // Adjust mainNav top position dynamically based on header-top height
            // This is important because header-top height can vary
            if (header.classList.contains('nav-open')) {
                const headerTopHeight = header.querySelector('.header-top').offsetHeight;
                mainNav.style.top = `${headerTopHeight}px`;
            } else {
                // If not open, allow CSS transition to slide it up, top set to 0 to align with header
                mainNav.style.top = '0'; // Reset to initial CSS position
            }
        });
    }
});