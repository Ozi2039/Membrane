...
function main() {
    ...
    async function loadAll() {
        ...
        list.innerHTML = "";
        const dueDate = await handleDueDate(sportsData);

        if (sportsData && sportsData.items) {
            sportsData.items.forEach(item => addItem(item.text, item.checked));
        }

        // Save updated due date if it was changed
        const dateString = dueDateHeader.textContent.replace("Date d'échéance : ", "");
        const currentDueDate = new Date(dateString).toISOString();
        if (!sportsData || sportsData.dueDate !== currentDueDate) {
            await saveAll();
        }
    }
